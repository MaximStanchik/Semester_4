...
namespace JewelleryStore
{
    public class LoginViewModel : PropertyChangedNotification
    {
        ...
        static string GetAccessByNickname(string connectionString, string nickname)
        {
            string access = null;
            ...
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                ...
                string query = "SELECT Access FROM USERS WHERE NickName = @Nickname";
        ...
        private void LogIn()
        {
            ...
            try
            {
                ...
                string access = GetAccessByNickname(connectionString, Username);


               
                if (access == "banned")
                {
                    YouAreBanned uarebannedWindow = new YouAreBanned();
                    uarebannedWindow.ShowDialog();
                    Environment.Exit(0);
                }
