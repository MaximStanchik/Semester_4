//рассказать про типы, нужно бдует создать массив
// создать тип для объекта, нужно будет разобраться с типами для функций

//----------Task_1----------//
function userInputLoop(): number[] {
    const userInputArray: number[] = [];
    

    for (let i = 0; i < userInputArray.length; i++) { 
        let userInput: string | null = prompt(`Enter an integer element from 0 to 9 (${i + 1}-st iteration):`);
        ...
        const element: number = parseInt(userInput, 10);
...
function toCreatePhoneNumber(arrayOfTenIntegers: number[]) {
...
//----------Task_3----------//
function toRotateArray(nums: number[], k: number): number[] {
...
//----------Task_4----------//
function toCalculateTheMedian(array1: number[], array2: number[]) {
    let median: number = 0;
...
function Main() {
    alert('First task');
    const inputArray: number[] = userInputLoop();
    ...
    const k_input: string | null = prompt('Please, enter K value:');

    if (k_input !== null) {
        const k: number = parseInt(k_input, 10);
